...
function loadEnv(overrides = {}) {
  ...
  return {
    ...
    APP_BASE_URL: merged.APP_BASE_URL || `http://localhost:${intFromEnv(merged.PORT, 3000)}`,
    DATABASE_URL: merged.DATABASE_URL || '',
    DATABASE_PATH: resolveProjectPath(merged.DATABASE_PATH, 'backend/data/pdv.sqlite'),
    DATABASE_SSL: boolFromEnv(merged.DATABASE_SSL, Boolean(merged.DATABASE_URL)),
    SESSION_COOKIE_NAME: merged.SESSION_COOKIE_NAME || 'big_lanche_session',
    CSRF_COOKIE_NAME: merged.CSRF_COOKIE_NAME || 'big_lanche_csrf',
    SESSION_TTL_DAYS: intFromEnv(merged.SESSION_TTL_DAYS, 14),
    PASSWORD_RESET_TTL_MINUTES: intFromEnv(merged.PASSWORD_RESET_TTL_MINUTES, 30),
    EMAIL_VERIFICATION_TTL_HOURS: intFromEnv(merged.EMAIL_VERIFICATION_TTL_HOURS, 24),
    BCRYPT_ROUNDS: intFromEnv(merged.BCRYPT_ROUNDS, 12),
    ENCRYPTION_KEY: merged.ENCRYPTION_KEY || '',
    SMTP_HOST: merged.SMTP_HOST || '',
    SMTP_PORT: intFromEnv(merged.SMTP_PORT, 587),
    SMTP_SECURE: boolFromEnv(merged.SMTP_SECURE, false),
    SMTP_USER: merged.SMTP_USER || '',
    SMTP_PASS: merged.SMTP_PASS || '',
    SMTP_FROM: merged.SMTP_FROM || 'Big Lanche <no-reply@biglanche.local>',
    ...
    MERCADO_PAGO_REDIRECT_URI: merged.MERCADO_PAGO_REDIRECT_URI || '',
    MERCADO_PAGO_AUTH_URL:
      merged.MERCADO_PAGO_AUTH_URL || 'https://auth.mercadopago.com/authorization',
    MERCADO_PAGO_OAUTH_TEST_TOKEN: boolFromEnv(merged.MERCADO_PAGO_OAUTH_TEST_TOKEN, false),
